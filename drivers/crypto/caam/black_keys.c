		   ...
		   u8 black_key_memtype, u8 keyauth, u8 trusted_key)
{
	...
	*black_key_length = key_length;
	...
	jstat = caam_jr_run_and_wait_for_completion(jrdev, coverdesc, caam_key_blob_done);
	if (jstat) {
			  ...
			  u8 black_key_memtype, u8 keyauth, u8 trusted_key)
{
	...
	*black_key_length = key_length;
	...
	retval = cnstr_desc_random_black_key(&coverdesc, key_length,
						black_key_dma,
						*black_key_length,
						keyauth, trusted_key);
	if (retval) {
