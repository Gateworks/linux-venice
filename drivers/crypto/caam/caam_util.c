...
void caam_key_blob_done(struct device *dev, u32 *desc, u32 err,
			       void *context)
{
 ...
 * @param[in]  size            The size
 */
void unprepare_read_data(struct device *jrdev, dma_addr_t dma_addr,
			 u8 *allocated_data, size_t size)
{
	/* Clear the data */
	memset(allocated_data, 0, size);
 ...
 */
dma_addr_t get_caam_dma_addr(const void *phy_address)
{
	uintptr_t ptr_conv;
