 ...
 */
int caam_blobencap(struct device *jrdev,
		    ...
		    u8 blob_memtype, u8 blobcolor)
{
	...
	*blob_length = secret_length_for_cnstr + BLOB_OVERHEAD;
	...
	jstat = caam_jr_run_and_wait_for_completion(jrdev, blob_encap, caam_key_blob_done);
	if (jstat) {
 ...
 * @return        0 if no error, otherwise error code
 */
int caam_blobdecap(struct device *jrdev,
		    ...
		    u8 auth, u8 trusted_key)
{
	...
	*secret_length = secret_length_req;
	...
	jstat = caam_jr_run_and_wait_for_completion(jrdev, blob_decap, caam_key_blob_done);
	if (jstat) {
